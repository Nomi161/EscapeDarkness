...
public class DoorController : MonoBehaviour
{
    ...
    MessageData message;            // 親小夫家区とがもっているScriptableObject情報を取得
        ...
        switch (roomData.roomName)
        {
            case "fromRoom1":
                if (GameManager.key1 > 0)   // 該当する鍵を持っていたら
                {
                    ...
                    GameManager.doorsOpenedState[0] = true; // 記録用の施錠情報をtrue
                }
                break;
            case "fromRoom2":
                if (GameManager.key2 > 0)   // 該当する鍵を持っていたら
                {
                    ...
                    nextTalk = true;        // 次のトーク展開をさせる
                    GameManager.doorsOpenedState[1] = true; // 記録用の施錠情報をtrue
                }
                break;
            case "fromRoom3":
                if (GameManager.key3 > 0)   // 該当する鍵を持っていたら
                {
                    ...
                    nextTalk = true;        // 次のトーク展開をさせる
                    GameManager.doorsOpenedState[2] = true; // 記録用の施錠情報をtrue
                }
                break;
        }
